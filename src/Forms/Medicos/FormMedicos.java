...
public class FormMedicos extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        TMedicos.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Nombre", "Apellido", "Nro Doc", "Telefono", "Email", "Especialidad", "Horarios"
            }
        ) {
      ...
      modelo.setRowCount(0);
        try {
            for (Medico medico : controller.findAll()) {
                String horarios = medico.getHorario_Desde() + " - " + medico.getHorario_Hasta();
       ...
       if (fila != -1) {
           ...
           try {
            ...
            (int) TMedicos.getValueAt(fila, 0),      // ID
                  ...
                  "", // Horario_Desde (se parsea después)
                  "" // Horario_Hasta (se parsea después)
        );
               
               String horarios = TMedicos.getValueAt(fila, 7).toString();
               if (horarios != null && horarios.contains(" - ")) {
                   String[] horariosArray = horarios.split(" - ");
                   objMedico.setHorario_Desde(horariosArray[0].trim());
                   if (horariosArray.length > 1) {
                       objMedico.setHorario_Hasta(horariosArray[1].trim());
                   }
               }
